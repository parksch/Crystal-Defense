...
public class JsonLoader : EditorWindow
{
    ...
    void GenerateScriptableManager(List<string> fileNames)
    {
        ...
        stringBuilder.AppendLine("            {");
        stringBuilder.AppendLine("                instance = FindAnyObjectByType<ScriptableManager>();");
        stringBuilder.AppendLine();
        stringBuilder.AppendLine("                if (instance == null)");
        stringBuilder.AppendLine("                {");
    ...
    string GenerateClassFromJson(JObject jsonObject, string className, bool isArray = true, bool isFirst = true, bool isFunction = false)
    {
        ...
        if (isFirst)
        {
            ...
            if (isArray)
            {
                ...
                {
                    stringBuilder.AppendLine($"        Dictionary<string, {className}> dataDict;");
                    stringBuilder.AppendLine();
                    stringBuilder.AppendLine($"        public {className} GetData(string key)");
                    stringBuilder.AppendLine("        {");
                    stringBuilder.AppendLine($"            if(dataDict == null)");
                    stringBuilder.AppendLine("            {");
                    stringBuilder.AppendLine($"                dataDict = new Dictionary<string, {className}>(); ");
                    stringBuilder.AppendLine();
                    stringBuilder.AppendLine($"                foreach (var item in {propertyName})");
                    stringBuilder.AppendLine("                {");
                    stringBuilder.AppendLine($"                    dataDict[item.index] = item;");
                    stringBuilder.AppendLine("                }");
                    stringBuilder.AppendLine();
                    stringBuilder.AppendLine("            }");
                    stringBuilder.AppendLine();
                    stringBuilder.AppendLine($"            return dataDict[key];");
                    stringBuilder.AppendLine("        }");
                }
